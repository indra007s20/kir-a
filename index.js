...
const { painmenu } = require('./src/painmenu')
...
case 'timer':
		...
		client.sendMessage(from, bahasa(prefix, sender), text, {quoted: mek})
				break
				case 'painmenu':
					client.sendMessage(from, painmenu(prefix, sender), text, {quoted: mek})
							break
							case 'menuadmin':
	...
	case 'kbbi':
	      ...
	      tels = body.slice(6)
			...
			if (isGroupMsg && isGroupAdmins) {
				if (args.length !== 1) return kill.reply(from, 'Você esqueceu de colocar se quer ativado [1], ou desativado [2].', id)
				if (args[0] == '1') {
					faki.push(chatId)
